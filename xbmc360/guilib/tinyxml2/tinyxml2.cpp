...
XMLError XMLDocument::LoadFile(CFile *file)
{
	Clear();
    ...
    {
        const long long fileLengthSigned = file->GetLength();//TIXML_FTELL( fp ); or file->GetPosition();
    ...
    const size_t read = file->Read(_charBuffer, 1, size);
    if ( read != size ) {
        SetError( XML_ERROR_FILE_READ_ERROR, 0, 0 );
        return _errorID;
    }
...
XMLError XMLDocument::SaveFile( CFile* fp, bool compact )
{
    // Clear any error from the last save, otherwise it will get reported
    // for *this* call.
    ClearError();
	//XMLPrinter stream( fp, compact ); WIP...
    //Print( &stream );
    return _errorID;
}
