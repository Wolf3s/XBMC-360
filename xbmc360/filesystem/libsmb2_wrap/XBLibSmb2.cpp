...
unsigned int CXBLibSMB2::Read(void *lpBuf, __int64 uiBufSize)
{
	...
	if(bytesRead < 0)
	{
		CLog::Log(LOGERROR, __FUNCTION__" - smb_read returned error %i", errno);
		return 0;
	}
...
unsigned int CXBLibSMB2::Write(void *lpBuf, __int64 uiBufSize)
{
	...
	if(byteswrite < 0)
	{
		CLog::Log(LOGERROR, __FUNCTION__" - smb_write returned error %i", errno);
		return 0;
	}
	return 0;
}
...
int CXBLibSMB2::Stat(const CURL &url)
{
	...
	if(stat < 0)
	{
		CLog::Log(LOGERROR, __FUNCTION__" - smb_stat returned error %i", errno);
		return 0;
	}
