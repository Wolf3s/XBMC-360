...
void CSpecialProtocol::SetXBMC360Path(const CStdString &dir)
{
  SetPath("xbmc-360", dir);
}
...
{
  return TranslatePath("special://xbmc-360") == TranslatePath("special://home");
}
...
CStdString CSpecialProtocol::ReplaceOldPath(const CStdString &oldPath, int pathVersion)
{
  if (pathVersion < 1)
  {
    if (oldPath.Left(2).CompareNoCase("P:") == 0)
      ...
      return URIUtils::AddFileToFolder("special://xbmc-360/", oldPath.Mid(2));
...
{
  CLog::Log(LOGNOTICE, "special://xbmc-360/ is mapped to: %s", GetPath("xbmc-360").c_str());
